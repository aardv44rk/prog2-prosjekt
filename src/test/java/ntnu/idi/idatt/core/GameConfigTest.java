...
public class GameConfigTest {
    ...
    public void testSaveAndLoadConfig() {
        try {
            ...
            assertEquals(5, loadedConfig.getPlayers().get(0).getPieces().get(0).getCurrentTile().getTileId(), 
    ...
    public void testSaveAndLoadPlayerList() {
        try {
            ...
            // Check that the file exists
            assertTrue(Files.exists(Paths.get(PLAYER_LIST_TEST_FILE)), "Player list file should be created");
            ...
            assertEquals(0, loadedPlayers.get(0).getPieces().size(), "Loaded players should have empty piece lists");
        } catch (IOException e) {
            fail("Exception should not be thrown: " + e.getMessage());
        }
    }
